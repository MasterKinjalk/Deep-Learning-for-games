...
class DQNAgent:
    ...
    def train_model(self):
        ...
        # train the model
        self.model.fit(
            current_state, target, batch_size=self.batch_size, epochs=1, verbose=0
        )
        self.update_epsilon()
    ...
    def train_model2(self):
        ...
        if done[i]:
            ...
            target_q_values[i][action[i]] = (
                reward[i] + self.discount_factor * max_qvalue_ns
            )
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                ...
                episode.append(e)
                print(
                    "episode: {}, score: {}, memory length: {}, epsilon: {}".format(
                        e, t, len(agent.memory), agent.epsilon
                    )
                )

                # store into file
                with open("cp_ddqn_bs24.txt", "a+") as file2:
                    file2.write(
                        "{}\t {} \t {:0.2f} \t {:0.2f}\t {}\n".format(
                            e, t, np.mean(Scores), np.mean(last100Scores), stepCounter
                        )
                    )
                break
